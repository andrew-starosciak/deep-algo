...
use algo_trade_token_selector::{TokenSelectionResult, TokenSelector};
use anyhow::Result;
use chrono::Utc;
use crossterm::{
...
impl ManagerApp {
    ...
    async fn refresh_dashboard_data(&mut self) -> Result<()> {
        ...
        let token_results = self.selector.get_selection_details(strategy_name).await?;
    ...
    async fn refresh_token_selection(&mut self) -> Result<()> {
        ...
        self.cached_token_results = self.selector.get_selection_details(strategy_name).await?;
...
fn render_dashboard(f: &mut Frame, app: &ManagerApp) {
    ...
    let scheduler_enabled = if app.config.backtest_scheduler.enabled {
        "✓ ENABLED"
    } else {
        "✗ DISABLED"
    };
    let scheduler_color = if app.config.backtest_scheduler.enabled {
        Color::Green
    } else {
        Color::Red
    };
    ...
    let scheduler_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                &app.config.backtest_scheduler.cron_schedule,
        ...
        Line::from(vec![
            Span::raw("Exchange: "),
            Span::styled(
                &app.config.backtest_scheduler.exchange,
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!(
                    "{} days",
                    app.config.backtest_scheduler.backtest_window_days
                ),
    ...
    let scheduler_panel = Paragraph::new(scheduler_lines).block(
        Block::default()
            .borders(Borders::ALL)
    ...
    let backtest_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                format!("{}h", app.config.token_selector.lookback_hours),
                Style::default().fg(Color::Yellow),
            ),
            Span::raw("): "),
            Span::styled(
                app.total_backtest_count.to_string(),
                Style::default()
                    .fg(Color::Green)
                    .add_modifier(Modifier::BOLD),
            ),
    ...
    let backtest_panel = Paragraph::new(backtest_lines).block(
        Block::default()
            .borders(Borders::ALL)
    ...
    let token_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                app.approved_token_count.to_string(),
                Style::default()
                    .fg(Color::Green)
                    .add_modifier(Modifier::BOLD),
            ),
        ...
        Line::from(vec![
            ...
            Span::styled(
                app.rejected_token_count.to_string(),
        ...
        Line::from(vec![
            ...
            Span::styled(
                (app.approved_token_count + app.rejected_token_count).to_string(),
                Style::default().fg(Color::White),
            ),
        ]),
    ];

    let token_panel = Paragraph::new(token_lines).block(
        Block::default()
            .borders(Borders::ALL)
    ...
    // Help
    let help =
        Paragraph::new("d: Dashboard | r: Reports | t: Token Selection | c: Config | q: Quit")
...
fn render_reports(f: &mut Frame, app: &ManagerApp) {
    ...
    let help = Paragraph::new(
        "↑↓: Navigate | Enter: View Detail | r: Refresh | Esc: Back to Dashboard | q: Quit",
    )
...
fn render_token_selection(f: &mut Frame, app: &ManagerApp) {
    ...
    // Title
    let title = Paragraph::new(format!(
        "Token Selection - {} ({} approved / {} total)",
        app.config.backtest_scheduler.strategy_name,
    ...
    // Criteria summary
    let criteria_lines = vec![Line::from(vec![
        Span::raw("Min Sharpe: "),
        Span::styled(
            format!("{:.2}", app.config.token_selector.min_sharpe_ratio),
            Style::default().fg(Color::Yellow),
        ),
        Span::raw("  Min Win Rate: "),
        Span::styled(
            format!("{:.0}%", app.config.token_selector.min_win_rate * 100.0),
            Style::default().fg(Color::Yellow),
        ),
        Span::raw("  Max DD: "),
        Span::styled(
            format!("{:.0}%", app.config.token_selector.max_drawdown * 100.0),
            Style::default().fg(Color::Yellow),
        ),
        Span::raw("  Min Trades: "),
        Span::styled(
            app.config.token_selector.min_num_trades.to_string(),
            Style::default().fg(Color::Yellow),
        ),
    ])];

    let criteria_panel = Paragraph::new(criteria_lines).block(
        Block::default()
            .borders(Borders::ALL)
    ...
    if app.cached_token_results.is_empty() {
        ...
        let header = Row::new(vec![
            "Status", "Symbol", "Sharpe", "Win%", "MaxDD%", "Trades", "PnL",
        ])
        .style(
            Style::default()
                .fg(Color::Yellow)
...
fn render_config(f: &mut Frame, app: &ManagerApp) {
    ...
    let scheduler_lines = vec![
        Line::from(vec![
            Span::raw("Enabled: "),
            Span::styled(
                app.config.backtest_scheduler.enabled.to_string(),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            Span::raw("API URL: "),
            Span::styled(
                &app.config.backtest_scheduler.hyperliquid_api_url,
                Style::default().fg(Color::Cyan),
            ),
        ]),
        Line::from(vec![
    ...
    let selector_lines = vec![
        ...
        Line::from(vec![
            Span::raw("Min Win Rate: "),
            Span::styled(
                format!("{:.1}%", app.config.token_selector.min_win_rate * 100.0),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                format!("{:.1}%", app.config.token_selector.max_drawdown * 100.0),
                Style::default().fg(Color::Yellow),
            ),
        ]),
        Line::from(vec![
    ...
    let selector_panel = Paragraph::new(selector_lines).block(
        Block::default()
            .borders(Borders::ALL)
...
fn render_report_detail(f: &mut Frame, app: &ManagerApp) {
    ...
    if let Some(report) = &app.detail_report {
        ...
        let detail_lines = vec![
            ...
            Line::from(vec![Span::raw(report.parameters.as_ref().map_or_else(
                || "  N/A".to_string(),
                |params| {
                    format!(
                        "  {}",
                        serde_json::to_string_pretty(params).unwrap_or_else(|_| "N/A".to_string())
                    )
                },
            ))]),
        ];
